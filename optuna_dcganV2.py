...
NZ, NC = 100, 1
NUM_EPOCHS = 40  
TRAIN_FRACTION = 0.2   #percentuale di dataset usata per il training
...
def objective(trial):
    ...
    beta1 = trial.suggest_float("beta1", 0.3, 0.45)
    lr_g = trial.suggest_float("lr_g", 1e-5, 3e-4, log=True)
    lr_d = trial.suggest_float("lr_d", 1e-5, 5e-4, log=True)
    lambda_l1 = trial.suggest_float("lambda_l1", 5.0, 10.0)
    lambda_mse = trial.suggest_float("lambda_mse", 2.0, 8.0)
    lambda_fm = trial.suggest_float("lambda_fm", 0.0, 3.0)
    ngf = trial.suggest_categorical("ngf", [64])
    ndf = trial.suggest_categorical("ndf", [128])
    update_d_every = trial.suggest_categorical("update_d_every", [5,7,11])
    dropout_p = trial.suggest_float("dropout_p", 0.3, 0.5)
    ...
    result_path = create_numbered_folder("OptunaResults15")
    ...
    # Log risultati
    with open("optuna_results15.txt", "a") as f:
